...
// ----- LazySegTree [Add & Maximum] (begin) -----

#ifndef QWERTYAM_LAZYSEGTREE_ADD_MAXIMUM_HPP
#define QWERTYAM_LAZYSEGTREE_ADD_MAXIMUM_HPP
...
private:
	...
	void update(const int& k) {
		...
		push(2 * k);
		push(2 * k + 1);
		val[k] = std::max(val[2 * k], val[2 * k + 1]);
	}
public:
	explicit LazySegTree_Add_Maximum(const std::vector<T>& vec) {
		...
		val = std::vector<T> (2 * n2, std::numeric_limits<T>::min());
	...
	T range_max(int left, int right) {
		...
		while (left < right) {
			if (left % 2 == 0) left /= 2;
				...
				push(left);
				l_res = std::max(l_res, val[left]);
				left = left / 2 + 1;
			}
			if (right % 2 == 0) right /= 2; 
			else {
				push(right - 1);
				r_res = std::max(val[right - 1], r_res);
				right /= 2;
			}
		}
	...
	T get(const int& index) {
		...
		return range_max(index, index + 1);
	}
};
